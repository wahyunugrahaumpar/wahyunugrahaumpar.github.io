  ...
  <nav class="navbar fixed-top navbar-expand-md navbar-light bg-warning sidebarNavigation pl-0 pr-0 pt-2 pb-2">
    <div class="navbar-brand label-lihat-hasil">Hasil Duel Acak</div>          
  ...
  <script type="text/javascript">
   ...
   if (gs('acak') == 'menunggu') {
       db.ref('duelacakmain/'+uid).once('value').then(function(snap){
        cetakHasil(snap.val(), uid);
       })
    }else{
       db.ref('duelacakmain/'+uidnya).once('value').then(function(snap){
        cetakHasil(snap.val(), uidnya);
       })
    }

    function cetakHasil(dataSoal, kode){
      var html = `<div class="text-center">
                  ...
                  </div><div><button class="btn btn-block btn-success m-lagi" onclick=tantangTeman('${uidnya}','${dataSoal[uidnya].nama}',${gs('deso')});>LAGI ?</button></div>`;
      ...
      acm=false;
        ...
        db.ref('duelacakmain/'+kode).once('value').then(function(snap){
          var data = snap.val();
          $.each(data, function(k,v){
            ...
            if(inde >= 2){
              inde=0;
              db.ref('duelacakmain/'+kode).remove();
            }
              setTimeout(function(){
                $('.m-lagi').fadeIn();
              }, 2000)
